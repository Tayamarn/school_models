...
PHI_DOMAINS = ('[0;90]', '[0;90]', '[180;270]', '[0;90]', '[0;90]')
ROTATION_PACKS = zip(S_MATRIXES, ROTATIONS, PHI_DOMAINS)
ROTATION_PACKS = {
    's_matrix': ROTATION_PACKS[0],
    'rotation_angles': ROTATION_PACKS[1],
    'phi_domain': ROTATION_PACKS[2]}
...
class Security(AbstractModel):
    def __init__(self, ml, team, logger, output):
        AbstractModel.__init__(self, ml, self.__class__.__name__,
                               team, logger, output)
        self.first_rotation_pack, self.second_rotation_pack = random.shuffle(
            ROTATION_PACKS)[:2]
        self.first_package = Package(self.first_rotation_pack)

        points = [random_vect for _ in range(2)]
        points.append(points[random.randint(1, 2)] * random.randint(2, 5))
        self.second_package = Package(
            self.second_rotation_pack, real_points=points)
    ...
    def team_arguments_without_control(self):
        return {
            'user_point': self.first_package.real_points[0],
            'first_key': self.first_package.real_points[1],
            'second_key': self.first_package.real_points[2],
            'cyphered_user_point': self.first_package.cypher_points[0],
            'first_cyphered_key': self.first_package.cypher_points[1],
            'second_cyphered_key': self.first_package.cypher_points[2]}

    def team_arguments_with_control(self):
        first_package_args = self.team_arguments_without_control()
        first_package_args['phi_domain'] = (
            self.first_package.rotation_pack['phi_domain'])

        second_package_args = {
            'user_point': self.second_package.real_points[0],
            'first_key': self.second_package.real_points[1],
            'second_key': self.second_package.real_points[2],
            'cyphered_user_point': self.second_package.crooked_cyphers[0],
            'first_cyphered_key': self.second_package.crooked_cyphers[1],
            'second_cyphered_key': self.second_package.crooked_cyphers[2],
            'phi_domain': self.second_package.rotation_pack['phi_domain']}
    ...
    def team_arguments(self, input_params):
        ...
        return self.team_arguments_without_control()
    ...
    def pre_production_without_control(self, model_params):
        ...
        max_angle_diff, work_correctness = self._max_angle_diff(
            player_angles, self.first_package.rotation_pack['rotation_angles'])
    ...
    def pre_production_with_control(self, model_params):
        ...
        for pack_name, pack_dict in clean_model_params.items():
            for key, val in model_params[pack_name]:
        ...
        computed_angles = (
            self.first_package.rotation_pack['rotation_angles'] +
            self.second_package.rotation_pack['rotation_angles'])
